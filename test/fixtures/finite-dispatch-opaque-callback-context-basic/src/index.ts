...
function process(schema: Schema, ctx: ProcessContext): string {
  ...
  const json = { live: "" };
  const params = { path: [] as (string | number)[] };

  if (schema._zod.processJSONSchema) {
    schema._zod.processJSONSchema(ctx, json, params);
  } else {
...
const opaque: Schema = {
  _zod: {
    def: { type: "boolean" },
    processJSONSchema(ctx, json) {
      json.live = `${ctx.target}:custom`;
    },
  },
  value: true,
};
