...
- targeted nested-callable follow-up date: `2026-05-15`
...
- A targeted rerun on `2026-05-15` after the public nested callable return-preservation change confirmed that the `src/v4/core/schemas.ts` `~standard.validate()` returned-carrier slice is no longer a live gap. Both owner-scoped anchors are now clean, lowering the live `unused-object-key` signal to `199` and the live `returned-object` signal to `2` while leaving the broader benchmark red on unrelated contract-sensitive and conservative families.
...
- Targeted follow-up outcome (`2026-05-15`) - benchmark still fails overall, but the owner-scoped `v4 ~standard validate wrapper` `mustNotFind` and `mustNotSkip` anchors are now clean. This narrows the live `unused-object-key` signal to `199` and the live `returned-object` signal to `2` without changing the remaining `standard-schema.ts` contract-sensitive families or the broader conservative boundary families.
...
- `unused-object-key` - baseline `59`, current `199`. The family is still mixed, and the nested public `~standard.validate()` returned-carrier anchor is now resolved. The remaining confirmed false positives are concentrated in the `allProcessors` dispatch-table flow, while most residual locale entries now verify as true or contract-sensitive compatibility labels rather than keyed-read misses.
...
- `returned-object` - baseline `3`, current `2`. The residual skip family shrank again after the nested public `~standard.validate()` preservation fix; the remaining skips stay outside that now-clean public callback slice.
...
- `unused-object-key` / `returned-object` on the public `~standard.validate()` path - the nested public callback false positives in `src/v4/core/schemas.ts` are gone and are now pinned by owner-scoped `mustNotFind` and `mustNotSkip` anchors.
...
- `unused-object-key` - 199 current unexpected findings. This family is mixed. The returned-carrier false positives are gone, including the public `~standard.validate()` slice; the remaining confirmed false positives are concentrated in the `allProcessors` dispatch table flow. Verified true detections include locale entries like `regex`, `starts_with`, `ends_with`, and `includes` in files that return early before the dictionary fallback. Contract-sensitive compatibility entries remain where locale dictionaries carry labels such as `uuidv4`, `uuidv6`, `template_literal`, and `mac` that the current `_issue.format` type surface does not emit.
...
- `returned-object` - 2 current unexpected skips. These remain the honest conservative boundary for bounded returned values whose later readback is not yet preserved through the exact subset after the public `~standard.validate()` path dropped out. The current bucket now also includes the spread-built `util.issue(...)` wrapper flow that no longer misreports direct dead fields but still stops at an explicit boundary.
...
1. Resolve the remaining bounded issue-array readback false positive so `unused-array-element` falls from `1`, and keep shrinking the non-locale `unused-object-key` subfamily now that the public `~standard.validate()` returned-carrier slice is resolved.
...
3. Tackle the conservative boundary families in explicit follow-up slices: `computed-property-access` (`970`), `object-spread` (`12`), `returned-object` (`2`), `array-spread` (`2`), `opaque-object-call` (`2`), `dynamic-array-index` (`2`), `array-opaque-mutation` (`1`), and `array-append-mutation` (`1`).
